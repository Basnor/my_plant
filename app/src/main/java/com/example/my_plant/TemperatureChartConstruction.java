...
public class TemperatureChartConstruction extends AppCompatActivity {
    ...
    public TemperatureChartConstruction() {
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final List<TemperatureChartConstruction.Data> finalData = getData();
    ...
    private List<TemperatureChartConstruction.Data> getData() {
        final List<TemperatureChartConstruction.Data> data = new ArrayList<>();
        ...
        for (int index = items.size() - 1; index >= 0; index--) {
            ...
            if (cal.get(Calendar.DAY_OF_MONTH) == day) {
                avgTemp += getItem(index, items).getTemperature();
                counter++;

            } else {
                String xVal = formXAxisValue(day, month);
                data.add(new TemperatureChartConstruction.Data((float) dayNum, (float) avgTemp / counter, xVal));
        ...
        String xVal = formXAxisValue(day, month);
        data.add(new TemperatureChartConstruction.Data(dayNum, (float) avgTemp / counter, xVal));
    ...
    private BarData generateBarData(List<TemperatureChartConstruction.Data> params) {
        ArrayList<BarEntry> values = new ArrayList<>();

        for (int i = 0; i < params.size(); i++) {

            TemperatureChartConstruction.Data d = params.get(i);
            BarEntry entry = new BarEntry(d.xValue, d.yValue);
            values.add(entry);
        }

        BarDataSet set1 = new BarDataSet(values, "Средняя температура за день (\u2103)");
    ...
    private LineData generateLineData(List<TemperatureChartConstruction.Data> params) {
        ...
        int temp = profile.getCollection().getTemperature();

        ArrayList<Entry> values1 = new ArrayList<>();

        for (int i = 0; i < params.size(); i++) {
            TemperatureChartConstruction.Data p = params.get(i);
            float val = (float) temp - (float) temp * 8 / 100;
        ...
        for (int i = 0; i < params.size(); i++) {
            TemperatureChartConstruction.Data p = params.get(i);
            if (i == 0) {
                values3.add(new Entry(p.xValue - 0.20f, temp));
            }

            values3.add(new Entry(p.xValue, temp));
        ...
        if (chart.getData() != null &&
            ...
            set1 = new LineDataSet(values1, "Диапазон нормальной температуры (\u2103)");
            ...
            // create a dataset and give it a type
            set2 = new LineDataSet(values2, "Диапазон нормальной температуры (\u2103)");
            set2.setLabel("");
            ...
            set3 = new LineDataSet(values3, "Норма средней температуры за день (\u2103)");
