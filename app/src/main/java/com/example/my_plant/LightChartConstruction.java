...
public class LightChartConstruction extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        chart = findViewById(R.id.light_chart);
        ...
        final List<LightChartConstruction.Data> finalData = getData();
    ...
    private List<LightChartConstruction.Data> getData() {
        final List<LightChartConstruction.Data> data = new ArrayList<>();
        ...
        for (int index = items.size() - 1; index >= 0; index--) {
            ...
            if (cal.get(Calendar.DAY_OF_MONTH) == day) {
                avgLight += getItem(index, items).getLight();
                counter++;

            } else {
                String xVal = formXAxisValue(day, month);
                data.add(new LightChartConstruction.Data((float) dayNum, (float) avgLight / counter, xVal));
        ...
        String xVal = formXAxisValue(day, month);
        data.add(new LightChartConstruction.Data(dayNum, (float) avgLight / counter, xVal));
    ...
    private BarData generateBarData(List<LightChartConstruction.Data> params) {
        ArrayList<BarEntry> values = new ArrayList<>();

        for (int i = 0; i < params.size(); i++) {

            LightChartConstruction.Data d = params.get(i);
            BarEntry entry = new BarEntry(d.xValue, d.yValue);
            values.add(entry);
        }

        BarDataSet set1 = new BarDataSet(values, "Средняя освещенность за день (%)");
    ...
    private LineData generateLineData(List<LightChartConstruction.Data> params) {
        ...
        int light = profile.getCollection().getLight();

        ArrayList<Entry> values1 = new ArrayList<>();

        for (int i = 0; i < params.size(); i++) {
            LightChartConstruction.Data p = params.get(i);
            float val = (float) light - (float) light * 8 / 100;
        ...
        for (int i = 0; i < params.size(); i++) {
            LightChartConstruction.Data p = params.get(i);
            if (i == 0) {
                values3.add(new Entry(p.xValue - 0.20f, light));
            }

            values3.add(new Entry(p.xValue, light));
        ...
        if (chart.getData() != null &&
            ...
            set1 = new LineDataSet(values1, "Диапазон нормальной освещенности (%)");
            ...
            // create a dataset and give it a type
            set2 = new LineDataSet(values2, "Диапазон нормальной освещенности (%)");
            set2.setLabel("");
            ...
            set3 = new LineDataSet(values3, "Средняя освещенность за день (%)");
